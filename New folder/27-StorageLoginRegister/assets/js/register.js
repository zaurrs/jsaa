    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        let passwordReply = document.querySelector("#passwordReply");
        let submit = document.querySelector(".btnn");
        let isLogined = false; // Yeni istifadəçi qeydə alındıqda daxil olmamış sayılır.
        ...
        function register(e) {
            ...
            // Şifrələr uyğun gəlirmi yoxlanır
            if (password.value !== passwordReply.value) {
                toast("Şifrələr uyğun gəlmir");
                return;
            }
    
            // İstifadəçi adı və ya email artıq mövcuddursa, qeydiyyatı etməyə icazə vermir
            let userUnique = users.some((user) => user.username === username.value || user.email === email.value);
            ...
            if (!userUnique) {
                ...
                users.push(newUser);
                localStorage.setItem("users", JSON.stringify(users)); // Yeni istifadəçini localStorage-a əlavə edirik
                toast("Qeydiyyat uğurla tamamlandı");
                setTimeout(() => {
                    window.location.href = "login.html"; // Qeydiyyat bitdikdən sonra login səhifəsinə yönləndirilir
                }, 2000);
            } else {
                toast("Bu istifadəçi adı və ya email artıq mövcuddur");
            }
        }
